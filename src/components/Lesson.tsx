        ...
        <header className='flex items-center justify-between'>
          {isLessonAvailable ? (
            <span className={`text-sm text-blue-500 font-medium flex items-center gap-2 ${isActiveLesson ? 'text-white' : ''}`}>
              <CheckCircle size={20} className={isActiveLesson ? 'text-white' : 'text-blue-500'} />
              Conteúdo liberado
            </span>
          ) : (
          ...
          <span className={`text-xs rounded px-2 py-[0.125rem] text-green-300 border border-green-300 font-bold ${isActiveLesson ? 'text-white border-white' : ''}`}>
            {props.type === 'live' ? 'AO VIVO' : 'AULA PRÁTICA'}
          </span>
        </header>
        <strong className={`text-gray-200 mt-5 ${isActiveLesson ? 'text-white' : ''}`}>
          {props.title}
        </strong>
      </div>
    </Link>
